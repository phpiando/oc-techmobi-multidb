...
use Techmobi\Multidb\Classes\Updater;
use Yaml;
...
class VersionManager
{
    ...
    /**
     * @var String Connection Name Custom
     */
    public $connection;
    ...
    public function removePlugin($plugin, $stopOnVersion = null, $stopCurrentVersion = false)
    {
        ...
        try {
            foreach ($pluginHistory as $history) {
                ...
                if (self::HISTORY_TYPE_COMMENT == $history->type) {
                    $this->removeDatabaseComment($code, $history->version);
                } elseif (self::HISTORY_TYPE_SCRIPT == $history->type) {
    ...
    {
        $versions = Db::connection($this->connection)->table($this->getTableName('system_plugin_versions'))->where('code', $pluginCode);
        if ($countVersions = $versions->count()) {
            $versions->delete();
        }

        $history = Db::connection($this->connection)->table($this->getTableName('system_plugin_history'))->where('code', $pluginCode);
    ...
    {
        if (null !== $this->fileVersions && array_key_exists($code, $this->fileVersions)) {
    ...
    protected function getDatabaseVersion($code)
    {
        if (null === $this->databaseVersions) {
            $this->databaseVersions = Db::connection($this->connection)->table($this->getTableName('system_plugin_versions'))->lists('version', 'code');
        }

        if (!isset($this->databaseVersions[$code])) {
            $this->databaseVersions[$code] = Db::connection($this->connection)->table($this->getTableName('system_plugin_versions'))
                ->where('code', $code)
                ->value('version');
        }
    ...
    protected function setDatabaseVersion($code, $version = null)
    {
        ...
        if ($version && !$currentVersion) {
            Db::connection($this->connection)->table($this->getTableName('system_plugin_versions'))->insert([
                'code' => $code,
                'version' => $version,
                'created_at' => new Carbon,
            ]);
        } elseif ($version && $currentVersion) {
            Db::connection($this->connection)->table($this->getTableName('system_plugin_versions'))->where('code', $code)->update([
                'version' => $version,
                'created_at' => new Carbon,
            ]);
        } elseif ($currentVersion) {
            Db::connection($this->connection)->table($this->getTableName('system_plugin_versions'))->where('code', $code)->delete();
        }
    ...
    {
        Db::connection($this->connection)->table($this->getTableName('system_plugin_history'))->insert([
            'code' => $code,
    ...
    {
        Db::connection($this->connection)->table($this->getTableName('system_plugin_history'))
            ->where('code', $code)
    ...
    protected function applyDatabaseScript($code, $version, $script)
    {
        ...
        $this->updater->connection = $this->connection;
        $this->updater->database = $this->databaseName;
        $this->updater->setUp($updateFile);

        Db::connection($this->connection)
            ->table($this->getTableName('system_plugin_history'))->insert([
            'code' => $code,
    ...
    {
        if (null !== $this->databaseHistory && array_key_exists($code, $this->databaseHistory)) {
            return $this->databaseHistory[$code];
        }

        $historyInfo = Db::connection($this->connection)->table($this->getTableName('system_plugin_history'))
            ->where('code', $code)
            ->orderBy('id')
            ->get()
            ->all();
    ...
    {
        return Db::connection($this->connection)->table($this->getTableName('system_plugin_history'))
            ->where('code', $code)
            ->orderBy('id', 'DESC')
            ->first();
    }
    ...
    protected function hasDatabaseHistory($code, $version, $script = null)
    {
        ...
        foreach ($historyInfo as $history) {
            ...
            if (self::HISTORY_TYPE_COMMENT == $history->type && !$script) {
                return true;
            }

            if (self::HISTORY_TYPE_SCRIPT == $history->type && $history->detail == $script) {
                return true;
            }
        }
    ...
    public function getCurrentVersionNote($plugin): string
    {
        ...
        $lastHistory = array_last(array_where($histories, function ($history) {
            return self::HISTORY_TYPE_COMMENT === $history->type;
        }));
